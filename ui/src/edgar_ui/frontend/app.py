...
from .api_client import EdgarAPIClient
...
def main() -> None:
    ...
    with st.spinner(f"Loading data for {ticker}..."):
        try:
            dl_result = client.download(ticker)
            if dl_result["status"] == "downloaded":
                st.success(f"Downloaded {dl_result['facts_count']:,} facts for {ticker}")
    ...
    _render_statement_tab(client, tab_income, ticker, "income", available["income"], period_value)
    _render_statement_tab(client, tab_balance, ticker, "balance", available["balance"], period_value)
    _render_statement_tab(client, tab_cashflow, ticker, "cashflow", available["cashflow"], period_value)
